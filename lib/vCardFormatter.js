...
(function vCardFormatter()
{
	...
	function getPhotoContentLine(prop, photo)
	{
		...
		if (majorVersion >= 4)
		{
			...
			if (photo.base64)
				return contentLine(prop, params, e("data:" + mediaType + ";base64," + photo.url));
		} else if (majorVersion === 3)
		{
			if (photo.base64)
				params.push("ENCODING=b");
			else
				params.push("VALUE=uri"); // a URI-referenced photo must declare VALUE=uri (RFC 2426 §3.1.4)
		...
		const line = contentLine(prop, params, e(photo.url));

		// vCard 2.1 mandates that an inline base64 value is terminated by a blank line;
		//   without it some parsers (notably Samsung devices) fail to import the card.
		//   v3.0/4.0 fold the value like any other text and their RFC grammars forbid
		//   blank lines, so the terminator is emitted for 2.1 only. See issue #58.
		if (photo.base64 && majorVersion < 3)
			return line + "\r\n";
